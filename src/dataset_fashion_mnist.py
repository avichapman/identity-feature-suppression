...
class DatasetFashionMNIST(Dataset):
    """
    Wrapper around the standard Fashion-MNIST dataset that allows us to introduce label noise.

    For symmetric and instance dependent noise, a label noise file is loaded and used.
    ...
    def __init__(self,
        ...
        if label_file_dir is not None:
            self.labels_dir = os.path.join(label_file_dir, "Fashion-MNIST")
        else:
            self.labels_dir = None

        dataset = torchvision.datasets.FashionMNIST(root=data_dir, train=train,
        ...
        self.data, self.samples_per_group, self.samples_per_label = \
            self._sort_data_into_groups(dataset.data,
                                        [label_id.item() for label_id in dataset.targets],
                                        corrupted_labels,
    ...
    def _add_label_noise(self, data: torchvision.datasets.FashionMNIST) -> List[int]:
        """
        Applies noise using the method selected in 'self.noise_method.

        :param data: A Fashion-MNIST dataset.
        ...
        if self.noise_method == LabelNoiseMethod.LABEL_NOISE_SYMMETRIC:
            ...
            raise ValueError('Asymmetric Noise is not supported with the Fashion-MNIST dataset.')
            ...
            raise ValueError('Open Set Noise is not supported with the Fashion-MNIST dataset.')
        else:
            return self._add_null_label_noise(data)

    @staticmethod
    def _add_label_noise_from_file(data: torchvision.datasets.FashionMNIST, file_path: str) -> List[int]:
    ...
    @staticmethod
    def _add_null_label_noise(data: torchvision.datasets.FashionMNIST) -> List[int]:
        """
        ...
        """
        return [label_id.item() for label_id in data.targets]
...
if __name__ == "__main__":
    _batch_size = 128
    _dataset = DatasetFashionMNIST(train=True,
                                   randomize=False,
                                   data_dir="./data",
                                   label_file_dir="./label_noise",
                                   noise_method=LabelNoiseMethod.LABEL_NOISE_INSTANCE_DEPENDENT,
                                   noise_level=40,
