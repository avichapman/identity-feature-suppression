...
class DatasetCIFAR10(Dataset):
    """
    Wrapper around the standard CIFAR10 dataset that allows us to introduce label noise.

    For symmetric, asymmetric and instance dependent noise, a label noise file is loaded and used. For open set noise,
    the dataset must be modified on the fly.
    ...
    def __init__(self,
        ...
        if label_file_dir is not None:
            self.labels_dir = os.path.join(label_file_dir, "cifar10")
        else:
    ...
    def _add_label_noise(self, data: torchvision.datasets.CIFAR10) -> List[int]:
        """
        Applies noise using the method selected in 'self.noise_method.

        :param data: A CIFAR10 dataset.
        ...
        if self.noise_method == LabelNoiseMethod.LABEL_NOISE_SYMMETRIC:
            ...
            label_file = os.path.join(self.labels_dir, f"inst.{self.noise_level}.{self.trial_index}.csv")
            return self._add_label_noise_from_file(data, label_file)
        elif self.noise_method == LabelNoiseMethod.LABEL_NOISE_OPEN_SET:
            return self._add_open_set_label_noise(data)
        else:
            return self._add_null_label_noise(data)

    @staticmethod
    def _add_label_noise_from_file(data: torchvision.datasets.CIFAR10, file_path: str) -> List[int]:
    ...
    def _add_open_set_label_noise(self, data: torchvision.datasets.CIFAR10) -> List[int]:
        """
        Applies open set noise in the manner described in:
        Wang et al., “Iterative Learning with Open-set Noisy Labels,” Proc. IEEE Comput. Soc. Conf. Comput. Vis.
        Pattern Recognit., pp. 8688–8696, 2018, doi: 10.1109/CVPR.2018.00906.

        A random number of samples from CIFAR100 are swapped into the dataset. The samples are taken from classes that
        are not represented in CIFAR10.

        :param data: A CIFAR10 dataset.
        :return: The corrupted labels for each sample in the dataset.
        """

        labels = data.targets  # The labels before any changes will be erroneous by the time we are done...

        # Download the CIFAR100 dataset...
        cifar100 = torchvision.datasets.CIFAR100(root='./data', train=True, download=True, transform=self.transform)

        # Exclude values that match the CIFAR10 classes...
        cifar100_data = cifar100.data

        # Select a random number of samples to be eligible for swapping...
        swap_mask = np.random.rand(len(data.targets)) <= self.noise_level / 100.

        # Pick random labels for each of the samples exposed by 'swap_mask'...
        data.data[swap_mask] = cifar100_data[np.random.choice(len(cifar100_data), swap_mask.sum())]

        # The actual truth of the swapped values is out of distribution and should be set to -1...
        targets = np.array(data.targets)
        targets[swap_mask] = np.ones(targets.shape)[swap_mask] * -1
        data.targets = [int(x) for x in targets]

        return labels

    @staticmethod
    def _add_null_label_noise(data: torchvision.datasets.CIFAR10) -> List[int]:
        """
...
if __name__ == "__main__":
    _batch_size = 128
    _dataset = DatasetCIFAR10(train=True,
                              randomize=False,
                              data_dir="./data",
