...
class DatasetMNIST(Dataset):
    """
    Wrapper around the standard MNIST dataset that allows us to introduce label noise.
    ...
    def __init__(self,
        ...
        if label_file_dir is not None:
            self.labels_dir = os.path.join(label_file_dir, "MNIST")
        else:
            self.labels_dir = None

        dataset = torchvision.datasets.MNIST(root=data_dir, train=train,
    ...
    def _add_label_noise(self, data: torchvision.datasets.MNIST) -> List[int]:
        """
        ...
        if self.noise_method == LabelNoiseMethod.LABEL_NOISE_SYMMETRIC:
            ...
            raise ValueError('Asymmetric Noise is not supported with the MNIST dataset.')
            ...
            raise ValueError('Open Set Noise is not supported with the MNIST dataset.')
        else:
            return self._add_null_label_noise(data)

    @staticmethod
    def _add_label_noise_from_file(data: torchvision.datasets.MNIST, file_path: str) -> List[int]:
    ...
    @staticmethod
    def _add_null_label_noise(data: torchvision.datasets.MNIST) -> List[int]:
        """
...
if __name__ == "__main__":
    _batch_size = 128
    _dataset = DatasetMNIST(train=True,
                            randomize=False,
                            data_dir="./data",
